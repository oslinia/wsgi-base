...
class TestModule(Response):
    ...
    def test_path(self):
        ...
        with self.assertRaises(ValueError) as context:
            Service(__name__, Map((Rule('', 'link'),)))
        ...
        with self.assertRaises(ValueError) as context:
            Service(__name__, Map((Rule('slash', 'link'),)))
        ...
        with self.assertRaises(ValueError) as context:
            Service(__name__, Map((Rule('/<int(1,2,3):name>', 'link'),)))
        ...
        with self.assertRaises(ValueError) as context:
            Service(__name__, Map((Rule('/<error:name>', 'link'),)))
    ...
    def test_token(self):
        ...
        with self.assertRaises(ValueError) as context:
            Service(__name__, Map((Rule('/<token>', 'link', {'token': (3, r'...')}),)))
        ...
        with self.assertRaises(ValueError) as context:
            Service(__name__, Map((Rule('/<path>', 'link', {'token': (3, r'...')}),)))
    ...
    def test_not_found(self):
        ...
        self.dummy_not_found(Service(__name__, not_found=dummy_not_found))
        self.dummy_not_found(Service(__name__, not_found=DummyNotFound))
        self.dummy_not_found(Service(__name__, not_found=(DummyNotFound,)))
        self.dummy_not_found(Service(__name__, not_found=(DummyNotFound, 'dummy_not_found')))
...
def service_tests():
    ...
    from .test_template import template_tests

    suite = unittest.TestSuite()
    suite.addTests(http_tests())
    suite.addTests(static_tests())
    suite.addTests(template_tests())
